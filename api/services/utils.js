...
module.exports = {
  ...
  discussionExists: function(discussionId) {
	...
	/**
	 * Retreives an avatar image from one of the following 
	 * services:
	 * 		- Gravatar
	 * 		
	 * @param  {string} service The name of a service.
	 * @param  {string} userId  The userId of the user.
	 * @param  {string} size    The size of the gravatar image.
	 * @return {string}         The url of the gravatar image.
	 */
	getAvatar: function(service, userId, size) {
		var url = '';

		switch(service) {

			case "gravatar":
				url = "http://www.gravatar.com/avatar/" + userId + "?s=" + size + "&d=identicon";
				break;

			default:
				url = sails.getBaseUrl() + '/images/avatar.svg';
  ...
  /**
   * Returns a promise that uploads a file.
   * @param  {File} file     File blob containing the file to upload
   * @param  {integer} maxBytes Maximum number of bytes for the file.
   * @return {Promise}          Returns a promise that uploads the file
   */
  uploadFile: function(file, maxBytes) {
    var maxBytes = maxBytes || 1024 * 1000 * 10; // default = 10 MB
    return new Promise(function(resolve, reject) {
      file.upload({maxBytes: maxBytes}, 
